...
public class Policies3 {
	...
	public boolean isPickUpPossible(int i, int j, int x) {
		if (x == 0) {
			if (i == 0 && j == 0 && Experiment3.PICK[0] > 0) {
				return (true);
			}
			if (i == 2 && j == 2 && Experiment3.PICK[1] > 0) {
				return (true);
			}
			if (i == 3 && j == 0 && Experiment3.PICK[2] > 0) {
				return (true);
			}
			if (i == 4 && j == 4 && Experiment3.PICK[3] > 0) {
				return (true);
			}
		}
		return (false);
	...
	public boolean isDropOffPossible(int i, int j, int x) {
		if (x == 1) {
			if (i == 4 && j == 0 && Experiment3.DROP[0] < 7) {
				return (true);
			}
			if (i == 3 && j == 3 && Experiment3.DROP[1] < 7) {
				return (true);
			}
	...
	public void updatePICKDROP(int operator, int i, int j) {
		if (operator == 4) {
			if (i == 0 && j == 0) {
				Experiment3.PICK[0] -= 1;
			}
			if (i == 2 && j == 2) {
				Experiment3.PICK[1] -= 1;
			}
			if (i == 3 && j == 0) {
				Experiment3.PICK[2] -= 1;
			}
			if (i == 4 && j == 4) {
				Experiment3.PICK[0] -= 1;
			}
		...
		if (operator == 5) {
			if (i == 4 && j == 0) {
				Experiment3.DROP[0] -= 1;
			}
			if (i == 3 && j == 3) {
				Experiment3.DROP[1] -= 1;
			}
		}
	}
	...
	public float findQFromTable(int[] state, int action) {
		...
		int qupdate = 0;
		Iterator itr = Experiment3.values.iterator();
		for (int l = 0; l < Experiment3.values.size(); l++) {
	...
	public void setQTable(int[] state, int action, float qValue) {
		...
		int qupdate = 0;
		int i = 0, j = 0, x = 0, action_code = 0;
		ListIterator itr = Experiment3.values.listIterator();
	...
	public void updateQActions() {
		int action_code;
		String action;
		for (int len = 0; len < Experiment3.values.size(); len++) {
			action_code = Experiment3.values.get(len).getAction_code();
			// System.out.println("Action : Code" + action_code);
			action = getAction(action_code);
			// System.out.println("Action " + action);
			Experiment3.values.get(len).setActions(action);
			// System.out.println("Action in QTable" +
			// Experiment3.values.get(len).getActions());
	...
	public int[] aplop(int i, int j, int x) {
		...
		if (x == 0) {
			...
			if ((i == 0 && j == 0 && Experiment3.PICK[0] > 0) || (i == 2 && j == 2 && Experiment3.PICK[1] > 0)
					|| (i == 3 && j == 0 && Experiment3.PICK[2] > 0) || (i == 4 && j == 4 && Experiment3.PICK[3] > 0)) {
				applicable_operators.add(k, 4);
				k++;
			}
		} else if (x == 1) {
			if ((i == 4 && j == 0 && Experiment3.DROP[0] < 8) || (i == 3 && j == 3 && Experiment3.DROP[1] < 8)) {
